...
const Workspace: React.FC<{ config: AppConfig; addLog: (log: any) => void }> = ({ config, addLog }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Security: Prevent extremely large files from exhausting browser memory (10MB limit)
  ...
  const processFile = async () => {
    ...
    try {
      ...
      addLog({
        ...
        confidence: 0,
        // Security: Log generic error message instead of raw exception details to prevent information leakage
