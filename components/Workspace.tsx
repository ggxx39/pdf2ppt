...
const Workspace: React.FC<{ config: AppConfig; addLog: (log: any) => void }> = ({ config, addLog }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Security: Limit file size to 10MB to prevent browser memory exhaustion
  ...
  const processFile = async () => {
    ...
    try {
      ...
      console.error("Internal processing error:", error); // Keep internal log for debugging
      setIsProcessing(false);
      addLog({
        step: "error_handler",
            ...
            <div className="w-full h-full relative group flex flex-col items-center justify-center">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity flex items-center justify-center gap-4 rounded-xl">
                <button 
                  ...
                  className="p-3 bg-red-500 text-white rounded-full hover:bg-red-600 focus-visible:ring-2 focus-visible:ring-red-400 focus-visible:ring-offset-2 shadow-lg transform hover:scale-110 transition-transform"
                >
                  <Trash2 size={24} />
                </button>
              </div>
