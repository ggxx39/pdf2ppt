...
const Workspace: React.FC<{ config: AppConfig; addLog: (log: any) => void }> = ({ config, addLog }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Security: Enforce 10MB file size limit to prevent client-side DoS (memory exhaustion)
  ...
  const processFile = async () => {
    ...
    try {
      ...
      console.error(error);
      setIsProcessing(false);
      // Security: Sanitize error messages in UI to prevent sensitive data leakage
      addLog({
        step: "error_handler",
